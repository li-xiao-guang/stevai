...
for epoch in range(epoch_num):
    ...
    for i in range(len(examples)):
        ...
        # hidden layer
        hidden = forward(example, input_hidden_weights)
        mask = np.random.randint(2, size=hidden.shape)
        hidden *= mask * 2

        # output layer
        prediction = forward(hidden, hidden_output_weights)
...
test_hiddens = forward(test_examples, input_hidden_weights)
test_predictions = forward(test_hiddens, hidden_output_weights)
