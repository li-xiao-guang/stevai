...
class Softmax(Layer):
    ...
    def __init__(self, axis=0.01):
        super().__init__()
        self.axis = axis

    def forward(self, x: Tensor):
        temp = np.exp(x.data)
        p = Tensor(temp / np.sum(temp, axis=self.axis, keepdims=True), requires_grad=True)

        def backward_fn():
            if x.requires_grad:
                x.grad = p.data * (p.grad - np.sum(p.grad * p.data, axis=self.axis, keepdims=True))
